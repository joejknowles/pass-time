...
export async function addSpecificDaySuggestions(
  dayOfWeekSuggestions: (TaskSuggestionConfig & { task: Task })[],
  userId: number,
  taskGroups: any[]
) {
  for (const suggestion of dayOfWeekSuggestions) {
    const relevantDayNumbers =
      relevantDayNumbersForSpecificDays[
        (suggestion.specificDays ||
          "SUNDAY") as keyof typeof relevantDayNumbersForSpecificDays
      ];
