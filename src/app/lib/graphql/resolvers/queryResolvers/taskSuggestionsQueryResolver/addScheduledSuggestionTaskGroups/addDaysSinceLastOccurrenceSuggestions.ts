...
export async function addDaysSinceLastOccurrenceSuggestions(
  suggestions: (TaskSuggestionConfig & { task: Task })[],
  userId: number,
  taskGroups: any[]
) {
    ...
    const aTaskDoneInLastXDays = await prisma.taskInstance.findFirst({
      where: {
        ...
        startTime: {
          gte: new Date(
            startOfToday -
              MS_IN_A_DAY * ((suggestion.daysSinceLastOccurrence || 3) - 1)
          ),
