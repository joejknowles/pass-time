...
export const calculateProgress = async (
  taskId: number,
  timeWindow: PROGRESS_TIME_WINDOWS,
  userId: number,
  depth: number = 0
): Promise<number> => {
  if (depth > 20) {
  ...
  if (timeWindow === "WEEKLY") {
    ...
    // starts Monday
    const numberOfDaysSinceStartOfWeek =
      dayNumberToday === 0 ? 6 : dayNumberToday - 1;
    timeRange = {
      gte: new Date(
        new Date().setHours(0, 0, 0, 0) -
          24 * 60 * 60 * 1000 * numberOfDaysSinceStartOfWeek
      ),
  ...
  for (const childTask of task.childTasks) {
    totalDuration += await calculateProgress(
      childTask.id,
      timeWindow,
      userId,
      depth + 1
