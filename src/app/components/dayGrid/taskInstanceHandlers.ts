...
export const moveTaskInstance = (
  ...
  if (movingTaskInfo && !movingTaskInfo.isSubmitting) {
    ...
    const originalTaskInstance = taskInstances?.find(
      (taskInstance) => taskInstance.id === movingTaskInstance.id
    ) as TaskInstance;

    const gridOffsetTop =
      document.querySelector("#day-grid-container")?.getBoundingClientRect()
        .top || 0;
    const containerHeight =
      document.querySelector("#day-grid-container")?.clientHeight || 1;
    const yPosition = clientY - gridOffsetTop;
    const THRESHOLD_OFFSET = 3;
    const cursorMinutesFromDaytimeStart =
      Math.floor(
        (((yPosition + THRESHOLD_OFFSET) / containerHeight) *
          daytimeHours.length *
          60) /
          15
      ) * 15;
    const preciseCursorMinutesFromDaytimeStart =
      ((yPosition + THRESHOLD_OFFSET) / containerHeight) *
      daytimeHours.length *
      60;

    const taskInstanceStart = movingTaskInstance.start;
    const taskInstanceMinutesFromDaytimeStart =
      taskInstanceStart.hour * 60 +
      taskInstanceStart.minute -
      daytimeHours[0] * 60;
    if (movingTaskInfo?.moveType === "end") {
      const newDuration = Math.max(
        cursorMinutesFromDaytimeStart - taskInstanceMinutesFromDaytimeStart,
        15
      );
      ...
      if (movingTaskInfo.moveType === "both") {
        if (!movingTaskInfo.cursorMinutesFromStart) {
          movingTaskInfo.cursorMinutesFromStart =
            preciseCursorMinutesFromDaytimeStart -
            taskInstanceMinutesFromDaytimeStart;
          setMovingTaskInfo({
            ...movingTaskInfo,
          });
        }
        startMinutesFromDaytimeStart =
          Math.floor(
            (preciseCursorMinutesFromDaytimeStart -
              movingTaskInfo.cursorMinutesFromStart) /
              15
      ...
      const minutesFromDayStart =
        daytimeHours[0] * 60 + startMinutesFromDaytimeStart;
      ...
      setMovingTaskInfo((state) =>
        state
          ? {
              ...
              isSameAsInitial:
                updatedDuration === originalTaskInstance.duration &&
