...
export const DayCalendar = ({
  ...
  const { currentDay, setCurrentDay, nowMinuteOfDay, isViewingToday } =
    useCurrentTimeAndDay();

  const [draftTaskInstance, setDraftTaskInstance] =
    useState<DraftTaskInstance | null>(null);

  const [isSubmittingTaskInstance, setIsSubmittingTaskInstance] =
    useState(false);

  const { isPhabletWidthOrLess } = useDevice();

  const [createTaskInstance, { error: errorFromCreatingTaskInstance }] =
    useMutation(CREATE_TASK_INSTANCE);
  const [updateTaskInstance, { error: errorFromUpdatingTaskInstance }] =
  ...
  const finalizeTaskInstance = useCallback(
    async (draftTaskInstance: DraftTaskInstance) => {
  ...
  if (errorFromGetTaskInstances || errorFromGetTasks) {
    return (
      <Typography color="error">
        Error loading data, please reload the page or contact us
      </Typography>
    );
  }
  ...
  if (errorFromCreatingTaskInstance || errorFromUpdatingTaskInstance) {
    return (
      <Typography color="error">
        Error saving data. Please refresh and try again.
      </Typography>
    );
  }
    ...
    >
      ...
      >
        ...
        >
          ...
          {isViewingToday && <CurrentTimeBar nowMinuteOfDay={nowMinuteOfDay} />}
        </Box>
      </Box>
