...
const TaskInstanceCard = ({
  ...
  const isThisCardSubmittingChanges =
    movingTaskInfo?.isSubmitting &&
    movingTaskInfo.taskInstance.id === taskInstance.id;
  const isThisCardInTouchEditMode =
    taskInstanceMovement?.taskInstanceInTouchEditMode === taskInstance.id;
  ...
  const touchDragIndicatorColor = `rgba(0, 0, 0, ${
    movingTaskInfo?.isSubmitting ? 0.4 : 0.7
  })`;
  ...
  return (
    <BasicTaskInstanceCard
      ...
      sx={{
        ...
        outline: isThisCardInTouchEditMode
          ? `2px solid ${touchDragIndicatorColor}`
          : undefined,
      }}
      onMouseDown={
        !isThisCardSubmittingChanges
          ? (event) => {
