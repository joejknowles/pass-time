...
interface EntityDetailsPanelProps {
  ...
  setOpenDetailsPanelEntity: (entity: OpenDetailsPanelEntity | null) => void;
...
const EntityDetailsPanel = ({
  ...
  const taskInstance = isTaskInstanceType
    ? taskInstances?.find((ti) => ti.id === openDetailsPanelEntity?.id)
    : null;
  const taskId = isTaskInstanceType
    ? taskInstance?.task.id
    : openDetailsPanelEntity?.id;
  const standardTask = isOpen
    ? tasks?.find((t) => {
        return t.id === taskId;
      })
    : null;
  const { data: detailedTaskData, refetch: refetchDetailedTask } = useQuery<{
    task: DetailedTask;
  }>(GET_TASK, {
    variables: { taskId },
  ...
  const handleGoToTaskDetails = (taskId: string, tab?: number) => {
    setPreviousEntities([
      ...previousEntities,
      openDetailsPanelEntity as OpenDetailsPanelEntity,
    ]);
        ...
        <TaskDetails
          ...
          goBack={
            previousEntities.length > 0
              ? () => {
                  const lastEntity =
                    previousEntities[previousEntities.length - 1];
