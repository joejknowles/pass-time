...
export const TaskInstanceDetails = ({
  ...
  const [deleteTaskInstance, { loading: isSubmittingDelete }] =
    useMutation(DELETE_TASK_INSTANCE);
  const [updateTaskInstance, { loading: isSubmittingInstanceUpdate }] =
    useMutation(UPDATE_TASK_INSTANCE);
  const [updateTask, { loading: isSubmittingTaskUpdate }] =
    useMutation(UPDATE_TASK);

  const isSubmitting =
    isSubmittingDelete || isSubmittingInstanceUpdate || isSubmittingTaskUpdate;
  ...
  const getDateTimeFromCustomTime = (customTime: {
    date: string;
    hour: number;
    minute: number;
  }) => {
  ...
  const getFormattedEndTime = () => {
    const end = new Date(
      getStartDateTime().getTime() + taskInstance.duration * 60 * 1000
    );
  ...
  const getFormattedDate = (dateTime: Date) => {
    ...
    return isToday
      ? `Today (${dateTime.toLocaleDateString(undefined, options)})`
      : dateTime.toLocaleDateString();
  };

  const suggestDefaultChange =
    taskInstance.duration !== (taskInstance.task.defaultDuration || 30);
        ...
        <Box
          ...
          onClick={() =>
            goToTaskDetails(
              ...
              TASK_DETAILS_TAB_INDEX.GENERAL
            )
          }
        >
              ...
              <Box sx={{ marginBottom: 2, display: "flex", gap: 2 }}>
                <FormControl sx={{ minWidth: 80, flexShrink: 0 }}>
                  ...
                  <Select
                    label="From"
                    value={`${
                      taskInstance.start.hour
                    }:${taskInstance.start.minute.toString().padStart(2, "0")}`}
                  ...
                  <FormControl sx={{ minWidth: 80, mr: 1 }}>
                    ...
                    <Select
                      ...
                      onChange={async (event) => {
                        ...
                        const duration =
                          (end.getTime() - getStartDateTime().getTime()) /
                          (60 * 1000);
                        await updateTaskInstance({
                          variables: {
                            input: {
                    ...
                    >
                      {Array.from({ length: 24 * 4 }, (_, i) => {
                        ...
                        const duration =
                          (endTime.getTime() - getStartDateTime().getTime()) /
                          (60 * 1000);
                        if (duration <= 0) {
                          return null;
                        }
                        ...
                        return (
                          ...
                          >
                            {`${endHour.toString().padStart(2, "0")}:${endMinute
                              .toString()
                              .padStart(2, "0")} (${formattedDuration})`}
                          </MenuItem>
                        );
                      })}
                    </Select>
