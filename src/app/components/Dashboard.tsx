...
const Dashboard = () => {
  ...
  const [openDetailsPanelEntity, setOpenDetailsPanelEntityRaw] =
    useState<OpenDetailsPanelEntity | null>(null);
  const [draggedTask, setDraggedTask] = useState<DraggedTask | null>(null);

  const setOpenDetailsPanelEntity = (
    ...
    if (newOpenEntity?.type === "TaskInstance" && isPhabletWidthOrLess) {
      let taskInstanceCard = document.getElementById(
        `task-instance-calendar-card-${newOpenEntity.id}`
      );
      if (!taskInstanceCard) {
        taskInstanceCard = document.getElementById(
          "new-task-instance-calendar-card"
        );
      }
      if (taskInstanceCard) {
        const dayCalendar = document.getElementById(
          "day-grid-scroll-container"
        );
        if (dayCalendar) {
